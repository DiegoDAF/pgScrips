...
_ as (
SELECT
  storage_settings.oid::regclass AS "rel",
  spc.spcname,
  to_char(c.reltuples, '9G999G999G999') AS "Rows",
  to_char(c.relpages, '9G999G999') AS "Pages",
  pg_size_pretty(8192::bigint*c.relpages) AS "Size",
  8192::bigint*c.relpages "size",
...
FROM
  ...
  JOIN storage_settings ON c.oid = storage_settings.oid
  JOIN pg_tablespace spc on c.reltablespace=spc.oid or (c.reltablespace=0 and spc.spcname='pg_default')
  /*LEFT JOIN pg_locks ON c.oid=pg_locks.relation WHERE pg_locks.relation is null*/
ORDER BY ice_in
)
select "rel", spcname, "Pages", "Size", "icedXID", ice_in, "IceAV%" from _
